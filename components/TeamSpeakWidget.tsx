...
export default function TeamSpeakWidget() {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setWatt((w) => {
        if (Math.random() < 0.08) return 70 + Math.round(Math.random() * 30); // af en toe schijven aan het werk: tot 100 W
        const richtingIdle = (46 - w) * 0.25; // zachtjes terug naar idle
        return Math.min(100, Math.max(38, Math.round(w + richtingIdle + (Math.random() * 6 - 3))));
      });
    }, 1600);
        ...
        <div className="w-72 rounded-lg border border-slate-700 border-t-2 border-t-lime-400 bg-slate-900 p-4 shadow-xl shadow-black/40">
          ...
          >
            ...
            <span className="relative mx-auto block w-fit transition-transform duration-300 group-hover:scale-[1.04]">
              ...
              {/* de strip loopt door het camera-perspectief licht schuin omhoog */}
              <span className="absolute bottom-[38.5%] left-[19%] flex origin-left rotate-[5deg] items-center gap-[2px]">
                {[0, 1, 2, 3, 4].map((i) => (
                  <span
                    key={i}
            ...
            <p className="mt-0.5 text-[10px] tabular-nums tracking-wide text-slate-500">
